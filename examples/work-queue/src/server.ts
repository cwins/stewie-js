// server.ts — HTTP server for the Work Queue example.
//
// Development: runs Vite as middleware, SSR-loads app.tsx on each request.
// Production: serves pre-built dist/client assets, imports compiled app.js.
//
// Version-sensitive layer: update this file when adapter or SSR API changes.
...
if (isProd) {
  // Production: serve pre-built assets from dist/client/
  ...
  server.listen(PORT, () => {
    console.log(`Work Queue (production) at http://localhost:${PORT}`);
  });
} else {
  // Development: Vite as middleware
  ...
  server.listen(PORT, () => {
    console.log(`Work Queue (dev) at http://localhost:${PORT}`);
  });
}
