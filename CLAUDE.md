...
**Why:** Module-scope reactive primitives become accidental singletons shared across requests in SSR environments. The compiler enforces this as a hard error; the runtime warns in dev mode. See `decision-records/0005-scoped-reactive-primitives.md`.
...
**Contract details settled during implementation** (promoted from agent memory 2026-08-11; verified against `packages/core/src/head.ts`):
- **`useTitle` does not restore the prior title on cleanup.** Deliberate: a stack-based save/restore is brittle under async navigation, where the unmount order of the outgoing route and the title write of the incoming one are not reliably sequenced. The last writer wins; routes are expected to set their own title.
- **`useMeta` has upsert-with-ownership semantics.** If a matching tag already exists in `document.head`, `useMeta` updates its `content` but does *not* take ownership — only the call that actually inserted the tag registers the `onCleanup` that removes it. This keeps a component from deleting a tag that the HTML shell or another component owns.
- **`useMeta` takes an object only** (`{ name, content }` or `{ property, content }`) — no positional overload. A positional form would be ambiguous about whether the first argument means `name` or `property`, and OpenGraph needs `property` while standard meta needs `name`. Identity key is whichever of the two is present.
...
- `Reactive<T>` — type-only alias (`() => T`) for typing reactive props; see the reactive-props decision below and `decision-records/0004`
- `defineResource(fn)` + `useResource(def, source)` with `AbortController` cancellation. Pass an explicit `{ id }` for SSR replay / cross-component dedup; unkeyed resources bypass the registry (STW063)
...
- **Decision-oriented docs — site LIVE (2026-08-04); guides to deepen** — a VitePress site is published at https://cwins.github.io/stewie-js/ via GitHub Actions (`.github/workflows/deploy-docs.yml`, base `/stewie-js/`), auto-deploying on push to `main` that touches `docs/**`. Home hero, nav/sidebar, local search; `pnpm docs:dev|build|preview` locally. Content: guides (getting-started, reactivity, routing, ssr, components, **stewie-way**), `docs/patterns/`, `docs/reference/` API pages. **Remaining:** deepen/complete the "Stewie way" decision guides (the publishing gap is closed). The field evidence that motivated this still stands — two canonical apps (Work Queue, external Pokemon demo) failed to discover shipped primitives (`useTitle`/`useMeta`/`<Head>`, `defineResource`/`useResource`, "component bodies are reactive scopes"); whether the live docs actually close that gap needs a fresh app built against them. See the "Discoverability of existing primitives" decision below.
- **Diagnostics — partial** — `DIAGNOSTICS.md` blueprints ~52 `STW###` codes; ~29 are implemented in package source today (STW001-007, 010, 011, 014, 020-022, 040-043, 050, 052, 063, 073, 083, 090-095, 100). A published diagnostics reference page (`docs/reference/diagnostics.md`) now backs every code's docs link. Remaining: ~21 codes (more compiler rules + dev-runtime warnings) and per-code silencing (deliberately deferred — see below). The dead docs-link bug (links pointed at a nonexistent domain) was fixed in 0.10.0.
...
- ~~**Compiler type awareness (DOM-emit text vs node)**~~ — Fixed in 0.10.0. The JSX-to-DOM emitter's `isProvenTextExpression` used to treat *any* zero-arg call as scalar text, so `<div>{makeFooter()}</div>` (a JSX helper) compiled to `nodeValue = String(makeFooter())` → `[object HTMLElement]` on the client (external bug report on 0.9.0). Now the checker is threaded into the emitter (`transformFile`/`findJsxReplacements`/`canTransformJsx`) and a child is emitted as a text node only when its resolved value type (return type for arrows) is provably primitive; otherwise the element falls back to the `jsx()` runtime, which renders the node structurally. `count(): number` stays text-optimized. Without a checker (standalone compiler, no program) it conservatively falls back — correctness over the micro-optimization; the Vite pipeline always has a checker.
...
- **`ErrorBoundary` does not catch re-render errors (NEW 2026-08-24)** — Open, deliberately deferred after the fix for the external Pokemon demo report. **This is a core renderer gap, not a routing one.**

  **What's true today.** `ErrorBoundary` is a one-shot `try`/`catch` around the *initial* `renderChildren` of its children (`dom-renderer.ts`, the `type === ErrorBoundary` branch). Every reactive update afterwards runs inside its own `effect()`, created long after that `try` returned. A throw during an update therefore escapes the effect entirely and propagates to **whatever wrote the signal** — `count.set(1)` throws at the call site. Verified two ways: a `<Show>` child that throws on a signal update (error surfaced at the writer, DOM kept stale content, boundary never engaged); and a route component that throws on mount under an `<ErrorBoundary>` wrapping `<Router>` (DOM left **completely empty**, since the outgoing subtree is removed before the incoming render throws, and `navigate()`'s promise rejected — which apps `void`, producing a bare `Uncaught (in promise)`).

  **Why it surfaced now.** It was the second bug in the Pokemon report: a param-derived computed threw during a route swap and the app was left with the URL changed and the old DOM mounted. The *trigger* is fixed (route params are render-scoped as of the 0.10.2 work), so that specific crash is gone. The containment gap is not — it applies to any app error during any update.

  **Options considered, with costs:**
  1. **Full fix.** Give `ErrorBoundary` runtime state (a signal holding the error) and a boundary stack that reactive effects capture at creation, mirroring how `_pushContext` / `_popContext` already work for providers; effects wrap their re-runs and route errors to the nearest boundary. Must discriminate thrown `Promise`s, because Suspense uses throwing as its protocol — a re-run that throws a Promise belongs to Suspense, not `ErrorBoundary`. Also needs a reset story (does a boundary ever flip back?). Touches `ErrorBoundary`, the reactive-child effect, Suspense interplay, and hydration. Redefines `ErrorBoundary`'s semantics, so it wants an ADR.
  2. **Router-local containment.** `try`/`catch` around `matchedContent`, falling back to the Router's existing `fallback` prop. Small and contained, fixes the blank-page symptom, but leaves every non-route reactive re-run unguarded — a bandaid by the "no layers of bandaids" standard.
  3. **Never strand, don't recover.** Keep the previous DOM on throw instead of leaving it empty, re-throw, warn in dev. Removes the blank-page failure and makes errors legible without introducing boundary state; doesn't foreclose option 1.

  **Decision (2026-08-24):** document and defer; ship the three fixes as 0.10.2 and design option 1 deliberately rather than under release pressure. The limitation is now stated in `docs/reference/core-api.md` with the recommended interim workaround (guard inside the reactive expression). Revisit before any claim that Stewie's error handling is production-grade — this is the gap behind such a claim.
...
- ~~**Reactive props across component boundaries**~~ — Settled 2026-07-13. See `decision-records/0004-reactive-props-and-the-Reactive-type.md` for the full analysis (incl. the preserved (b)/(c) gotcha catalog, the publish-format question, and the 2026-05-28 canonical-app observations).
  ...
  **Decision:** keep accessor-typed props (option a) and make them ergonomic + honest rather than removing the `() => T` from signatures.
  ...
  - **`type Reactive<T> = () => T`** ships from `@stewie-js/core` (type-only, no runtime surface). `Reactive<string>` reads as intent where bare `() => string` does not. Signals/computeds are callable, so they're already assignable to `Reactive<T>`; store fields connect via a thunk. It is a *plain structural alias* — it grants no extra type-system power (structurally identical to `() => T`). A TypeChecker spike validated that its **alias symbol** is detectable (survives generic instantiation + cross-module import), but that technique is currently unused now that both auto-wrap and STW033 are dropped (see next bullet); it's kept in the ADR as a proven building block.
  - **No compiler auto-wrap for component props** (a type-aware wrap was planned, then dropped 2026-07-21 before implementation). It has no valid use case under accessor-only typing: a `Reactive<T>` prop accepts only `() => T`, so anything editor-valid is already an accessor (nothing to wrap), and the only wrap-targets (bare values) are editor type errors. Intrinsic attributes auto-wrap only because they're union-typed (`string | (() => string)`) — we chose *not* to union-type component props. The idiom is explicit accessors (`name={signal}`, `name={() => expr}`); always correct, compiler-optional by construction. (A `Reactive` alias-symbol detection technique was validated by a TypeChecker spike but has no consumer now that both auto-wrap and STW033 are dropped.)
  - **Branding declined** (would force stamping signals/thunks and pull toward compiler-required). **STW033 dead-thunk lint descoped** (2026-07-21) — undecidable: a plain function and a reactive accessor are type-identical (`() => T`), so any rule flagging `() => getRandomName()` also fires on idiomatic accessor-chaining (`() => usersById()[id]`), e.g. Work Queue's `TaskRow`. TypeScript already catches the *decidable* mistakes (a value or snapshotted `signal()` where `() => T` is expected), so reactive props need no compiler support at all — the type + convention are the whole feature.
...
- **Discoverability of existing primitives (NEW 2026-05-28)** — Open. Two canonical apps (Work Queue and the external Pokemon demo) have now been built. *Neither used `useTitle` / `useMeta` / `<Head>`* — both fell back to `effect(() => { document.title = ... })`. The Pokemon writeup explicitly says "there is no clear 'head management' abstraction in use" and requests "a simple title/meta API" — a feature we shipped but they could not find.
  ...
  **Update (2026-08-04):** the docs site is now **live** at https://cwins.github.io/stewie-js/ (VitePress via GitHub Pages), and the "Stewie way" guide covers head/title, loader-vs-`useResource`, and "component bodies are reactive scopes" — directly targeting the three failures above. The publishing gap is closed. Still not *proven* to close the discoverability gap — that needs a fresh app built against the live docs — but the missing content now exists and is discoverable. Remaining work is deepening the decision guides.

- ~~**View Transitions and scroll restoration coherence**~~ — Settled and shipped in 0.9.0. See `decision-records/0008-route-direction-is-structural.md`. After a design pass and rubber-wall review the v1 contract is:
