...
- ~~**Actions / mutations API shape**~~ — Settled. Ship `defineAction(fn)` + `useAction(def)`. `defineAction` returns an opaque `ActionDefinition<I, O>` that creates no signals (safe at module scope); `useAction` is a free function (matches `consume(Context)`, not `def.use()`) that creates the per-component `{ run, pending, error, reset }` instance. `run` no-ops while pending and resolves with `undefined`; on error it sets `error` to the caught `Error` and resolves with `undefined`; on success it resolves with the action's return value. Each new `run` batches `pending=true, error=null` at start. `reset()` clears `error`, no-op while pending. No cancellation in v1. Post-mutation work (navigation, store sync, toasts) lives in caller code after `await submit.run()` — no lifecycle callbacks. The flat `action()` prototype at `packages/core/src/action.ts` is to be reshaped, not extended. See ROADMAP.md "Actions / Mutations" for the full settled spec.
...
- **Resource primitive shape** — The current flat `resource(source, fetcher)` will be reshaped to `defineResource(fn)` + `useResource(def, source)` for consistency with `defineAction`/`useAction` and to eliminate the same module-scope footgun. The reactive-triggering asymmetry stays (resources fire on source change; actions fire on `.run()`) — that's intrinsic to what each primitive is. Existing callers will be updated in the same change. The word "resource" is kept for now; revisit a broader rename only if a clearly better word emerges.
...
- **Data cache / query layer** — `resource()` (post-reshape) plus route loaders covers the common case well. Do not rush toward a TanStack-Query-style cache. Revisit only if the canonical app hits concrete ceilings that `resource` + loaders cannot address. A full cache layer risks making the API surface sprawling.
