...
- **Decision-oriented docs — site built, publish pending; guides to complete** — a VitePress site now exists under `docs/` (home hero, nav/sidebar, local search; `pnpm docs:dev|build|preview`), with guides (getting-started, reactivity, routing, ssr, components, **stewie-way**), `docs/patterns/`, and `docs/reference/` API pages. A GitHub Pages deploy workflow (`.github/workflows/deploy-docs.yml`, base `/stewie-js/`) is wired and builds green. **Remaining:** (1) the user must enable Pages in repo settings (source = GitHub Actions) for it to go live; (2) complete/deepen the "Stewie way" decision guides. The field evidence that motivated this still stands — two canonical apps (Work Queue, external Pokemon demo) failed to discover shipped primitives (`useTitle`/`useMeta`/`<Head>`, `defineResource`/`useResource`, "component bodies are reactive scopes"). See the "Discoverability of existing primitives" decision below.
...
- **Discoverability of existing primitives (NEW 2026-05-28)** — Open. Two canonical apps (Work Queue and the external Pokemon demo) have now been built. *Neither used `useTitle` / `useMeta` / `<Head>`* — both fell back to `effect(() => { document.title = ... })`. The Pokemon writeup explicitly says "there is no clear 'head management' abstraction in use" and requests "a simple title/meta API" — a feature we shipped but they could not find.
  ...
  **Update (2026-08-03):** the docs site is now built (VitePress) and the "Stewie way" guide exists with sections on head/title, loader-vs-`useResource`, and "component bodies are reactive scopes" — directly targeting the three failures above. Not yet *proven* to close the gap (needs publish + a fresh app built against the live docs), but the content that was missing now exists. Remaining discoverability work is publishing (enable Pages) and deepening the decision guides.
