...
- **Actions / mutations API shape** — An unshipped prototype lives at `packages/core/src/action.ts` (not exported from `@stewie-js/core`). Settled so far: `pending` is strictly bounded by the mutation itself; the framework does not interpret results (success is observable via `error()`, which is `''` on success); post-mutation work is caller code after `await run()`, not lifecycle callbacks; per-site pending isolation comes from per-component instantiation. Open: (1) the prototype's flat `action(fn)` shape has a module-scope footgun — safe reuse requires a `defineAction(fn)` + `.use()` split so shared action definitions create no signals at module scope; (2) naming (`action` is overloaded by MobX/Redux/Remix); (3) relationship to form primitives (absorb submit-level tracking into `createForm()` or keep separate); (4) a possible future client-side action-routes layer using a context registry for runtime DI (not server/RPC — the URL is type-indexed identity). See ROADMAP.md "Actions / Mutations" for full detail. Do not export the prototype until these are resolved.
