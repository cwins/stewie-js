...
- **Reactive props across component boundaries** — Open, parked 2026-05 after a detailed design pass on option (c) surfaced enough load-bearing gotchas to justify waiting for concrete pressure before committing.

  Today, passing live data into a child component requires accessor-typed props: parent writes `<Child name={() => episode().name} />` and child types `name: () => string` and reads `props.name()`. This contradicts the stated principle that the developer should not have to write code differently for reactivity to work — every component author has to think about which props might be reactive and pollute their type signatures with `() => T`.

  **Three known design points:**
  (a) **Accessor-typed props** (current state — honest, verbose, contagious). Every prop's reactivity is a public-API decision; changing static→reactive is a breaking change to consumers; libraries that want both end up typing as `T | (() => T)`.
  (b) **Proxy props** à la Solid, where `props` is a Proxy that intercepts property access and tracks the read. `props.name` stays live but destructuring silently breaks reactivity (`const { class: cls, ...rest } = props` — the most common pattern in headless/forwarding components). Solid ships `mergeProps`/`splitProps` as a permanent ecosystem-wide workaround.
  (c) **Compiler-rewritten access** — author writes `props.name` and `<Child name={value} />` with plain types; compiler instruments both ends so reactivity flows transparently. Runtime contract sketched: value-typed props are thunks (`() => T`), function-typed props are the function itself; type and runtime shapes diverge by one thunk indirection that the compiler maintains. SwiftUI's `@Observable` / `@State` / `@Binding` is the inspiration: macros + property wrappers make observation transparent at the call site.

  **Gotchas catalogued when sketching (c) in detail — do not lose this list when revisiting:**
  - **Override-merge spread is genuinely hard.** `const merged = { ...props, label: 'x' }; <Inner {...merged} />` — `merged` mixes props-bag fields (already thunked) with plain literal fields (not). Origin-based pass-through breaks reactivity for the literal field; type-driven thunking re-wraps already-wrapped fields. This is the case Solid's `mergeProps` exists for, and "(c) eliminates the need for mergeProps" is false until this is specified.
  - **Refs** (`ref={...}`) are callback-or-object unions — neither value-typed nor function-typed in the binary the contract assumes. Needs an explicit third category.
  - **Polymorphic `as` prop** (`<Box as={Heading} />`) — `as` is value-typed (a `ComponentType`), but the child-site rule would rewrite reads to `props.as()`, which is wrong. Value-typed-thunk is not a clean rule; there is at least one exception (component-type values), probably more.
  - **Static JSX children of components** (`<Wrapper><div /></Wrapper>`) — under "all value-typed props are thunks", children become thunks too, which breaks every existing `props.children` read. Either children are a permanent exception to the rule, or the rule itself isn't "all value-typed → thunk."
  - **Destructuring depth is Solid's line, not a universal one.** "Destructure the props bag = reactive; destructure a value from props = snapshot" maps to Solid's specific rule, which Solid users still trip on constantly. Compose/MobX draw different lines. Don't claim universality.
  - **`as any` semantics.** Origin tracking through `as any` was hand-waved as "the author probably meant to silence an unrelated type error." Compiler rules should not depend on intent. Either origin is purely syntactic (cast opaque) or it sees through casts and owns the false-positive case. Pick one.
  - **`visitJsxChildren` change is more than removing the intrinsic guard.** `analyzer.ts:283`'s `if (!isIntrinsicElement(tagName)) return;` exists *because* component children flow to `props.children` and the wrap point is at the consumer, not the JSX site. The real change is "decide which side owns the wrap for component children" — call-site wrap is redundant if child-site reads are rewritten. Asymmetric with intrinsics, not a clean guard split.
  - **First-encounter footgun on CDN/sandboxes.** Even if `.jsx` libraries load via esm.sh (generic JSX→`_jsx` transform), the Stewie compiler doesn't run, so prop instrumentation is skipped. Components render once, reactivity dies silently. A new user trying Stewie in a non-WebContainers sandbox hits broken reactivity with no obvious error. A dev-mode runtime warning catches the case where a function is passed as a value-typed prop, but doesn't catch the case where `count()` was unwrapped at the JSX site before the boundary.
  - **Production-build safety needs more than a runtime marker.** A `__STEWIE_COMPILED__` per-module marker plus a dev console warning is insufficient — bundles can mix compiled and uncompiled modules. A Vite-plugin-level production-build assertion that errors if any reachable `.jsx`/`.tsx` was processed without the Stewie transform is the only thing that prevents silently shipping broken reactivity to users.
  - **Compiler-load-bearing posture shift is bigger than a wording tweak.** "Compiler is optional" is wired into Bet #1's identity. Moving to "compiler is canonical, required for idiomatic component composition" is a real shift and should be argued, not absorbed.

  **Tightly-coupled but separately-evaluable question: library publish format.** Option (c) only delivers its ergonomic win if the compiler can see into third-party libraries. That requires libraries to publish **preserved JSX** (`tsc` with `jsx: "preserve"` outputs `.jsx` + `.d.ts`; consumer's Vite plugin compiles everything including `node_modules`). This is the Svelte/Vue SFC ecosystem model and avoids the React Compiler's node_modules-is-a-black-box limitation. It has its own gotchas: in-browser/CDN sandboxes can't load `.jsx` directly (SyntaxError on unpkg/jsDelivr, or silent reactivity loss via esm.sh's generic transform); `.d.ts` JSX namespace versioning across workspace deps requires consumer and library to agree on `@stewie-js/core`'s JSX namespace version; HMR through `node_modules` requires Vite watcher configuration. A future `@stewie-js/compiler/jit` CDN bundle (analogous to Angular's legacy JIT compiler) is *possible* as a documented dev-only convenience but is not v1 and not production-recommended. Preserved JSX is also potentially useful in an accessor-typed-prop world — it lets the compiler's auto-wrap pass reach into libraries — so the publish-format question can be evaluated on its own merits when there's pressure for it.

  **Tripwire for revisiting:** Pick (a), (b), or (c) when the canonical Work Queue app shows multiple cases where accessor-typed props feel painful in real component composition, or when a third-party library author hits the contagion firsthand and reports it. When revisited, the gotchas catalogued above are the work that has to be done concretely, not handwaved — particularly override-merge spread, refs, polymorphic `as`, static component children, and the production-build assertion. Option (c) is not "almost done"; it is "designed enough to know the cost is high."

  **Canonical-app observations (2026-05-28), Work Queue Steps 2–4:** Five new prop-passing boundaries landed (`UserChip`, `TaskRow`'s `usersById`, `UserPicker` at three task sites and two project sites). Concrete friction points worth pinning down before they evaporate:

  - **Accessor types are spreading at every reusable-component boundary.** `UserPicker.value: () => string | null`, `UserChip.user: UserPublic | null | (() => UserPublic | null)` (carrying `T | (() => T)` exactly as predicted in option (a)'s description), `TaskRow.usersById: () => Record<string, UserPublic>`. Each new reusable component made the type signature one step more reactive-aware. None of these would be reactive in option (b) or (c) — the author would write `string | null`, `UserPublic | null`, `Record<string, UserPublic>` and reactivity would flow.
  - **`UserChip` ships `T | (() => T)` as its public API** and uses a `resolve()` helper to normalise. This is the exact contagion shape option (a)'s entry warned about. Two consumers (TaskRow, ProjectDetailPage's lead chip) already exercise both arms — the static-value arm in the lead chip, the accessor arm in TaskRow. A third consumer that wanted reactivity but received the static-arm by mistake would silently lose updates.
  - **Function-children-as-array workaround is unsightly.** Mapping a list inline (`<option>` per user) hits `JSXChild[] not assignable to JSXChild` because nesting an array among siblings exceeds the children type. Current pattern: `{() => users.map((u) => <option ...>...</option>)}`. This isn't reactive-props per se, but it is the most visible "code I had to write differently because of reactivity" moment in the whole app. It survives in `UserPicker` and the assignee selects. A typed `Fragment`/`Children` improvement may be cheaper than the full reactive-props rewrite and worth considering in isolation.
  - **Project-lead chip needed an IIFE in JSX to derive-then-render** (`{(() => { const lead = ...; return <Link>...</Link>; })()}`). This is the "I want a local binding inside JSX" case that destructuring-from-props in option (b) makes natural. Solid users hit this often.
  - **`peek()` everywhere in action `run({...})` payloads is fine and explicit, not painful.** The current pattern of `$title.peek()` etc. at the snapshot boundary reads cleanly; nobody had to think about whether the value was reactive. The friction is one layer up — at *component composition*, not at *signal usage inside one component*.

  Net: the tripwire ("multiple cases where accessor-typed props feel painful in real component composition") is now arguably met for the *contagion type-signature* axis. It is not yet met for the *destructuring-loses-reactivity* axis (no destructured-prop bugs encountered). When this is revisited, the path forward is probably narrower than "pick (a), (b), or (c) wholesale" — option (c) might be staged so it addresses the public-API contagion (the most visible cost) without committing to compiler-required-everywhere semantics (the most expensive gotcha cluster).

  **External-build observation (2026-05-28), Pokemon demo:** A second app built by an external agent (interactive Pokedex with route loaders, SSR tests, transitions, query-backed search) did *not* flag accessor-typed props as friction. Pages used `signal` / `computed` / `For` / `Show` / `Switch` heavily but extracted very few reusable cross-page components (no `UserPicker`-equivalent appeared). Their writeup's "things to improve" list does not mention reactive props at all. Useful counter-data: the contagion shows up at *reuse boundaries*. A render-heavy app whose components stay page-local can stay in option (a) without feeling it. This sharpens when to spend the cost — it is paid by frameworks targeting library authors and shared component kits, not by app authors writing one-off pages.

- **Query-state ergonomics in the router (NEW 2026-05-28)** — Open. The Pokemon demo's biggest friction was that calling `useRouter().navigate()` (or any router-level query mutation) on every keystroke caused input focus loss and apparent route remounting. They abandoned reactive query state entirely and fell back to `window.history.replaceState(null, '', nextUrl)` plus a hand-rolled `buildDiscoverUrl()` URL builder, only committing query to the URL when the user pressed Search. They list this as "the single biggest source of friction in the app" and request a first-class `setQuery()` API with explicit push/replace and a guarantee the route does not remount for query-only changes.

  This is high-signal. URL-backed search/filter UIs are a flagship router use case; if the router remounts the page tree on a query-only nav, every searchable page hits this. Likely a router behavior issue (loader re-runs and/or component re-mounts when only the query string changed), not a docs gap.

  Tripwire reached. The right next step is a router behavior audit: when does `navigate()` re-run loaders, when does it re-mount components, and what is the contract for query-only updates? Probably want `router.setQuery(patch, { replace?: boolean })` as a first-class API that explicitly does *not* trigger a re-mount or loader re-run for query-only changes, with `useQuery()` already reading reactively from the underlying location signal.
