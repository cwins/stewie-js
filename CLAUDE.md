...
- **Suspense + hydration correctness** — Open until DataRegistry lands. Today `renderSuspense` doesn't engage the `HydrationCursor` at all; on hydration it creates a fresh anchor and re-renders children. If children re-throw on hydration (because resource data wasn't replayed), the fallback flashes back in. Fix has three coordinated parts: (1) SSR emits a `<!--Suspense-->` anchor at end of boundary content, (2) `renderSuspense` claims via `collectUntilComment('Suspense')` like `renderShow` does, (3) `useResource` reads from the DataRegistry so SSR-resolved data doesn't re-throw. Part (3) is load-bearing — without it (1)+(2) don't actually solve the user-visible flash.
