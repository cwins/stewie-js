...
18. **Diagnostics — dev-mode and build-time** *(partial)* — `DIAGNOSTICS.md` blueprints ~52 `STW###` codes; ~29 are implemented in source today (10 new in 0.10.0: STW020/021/041/043/050/063/090/091/093/100). The docs page mapping each code to a fix is DONE (`docs/reference/diagnostics.md`, links wired via `diagnosticDocsUrl`). Remaining: ~21 more codes (compiler rules + dev-runtime warnings) and per-code silencing (deferred — the compile-time vs dev-runtime lever design isn't settled)
