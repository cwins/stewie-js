...
import { signal, computed, effect, batch, untrack, createRoot, createScope, withRenderIsolation, stableList } from './reactive.js';
...
describe('stableList', () => {
  it('derives an array from reactive source', () => {
    createRoot(() => {
      const items = signal([{ id: 1 }, { id: 2 }]);
      const list = stableList(() => items().filter((i) => i.id > 0));
      expect(list()).toHaveLength(2);
    });
  });

  it('returns same array reference when item refs are unchanged', () => {
    createRoot(() => {
      const a = { id: 1 };
      const b = { id: 2 };
      const src = signal([a, b]);
      const list = stableList(() => src());

      const first = list();
      // Replace signal value with new array containing same object references
      src.set([a, b]);
      expect(list()).toBe(first); // same reference — no downstream notification
    });
  });

  it('returns new array reference when an item reference changes', () => {
    createRoot(() => {
      const a = { id: 1 };
      const b = { id: 2 };
      const src = signal([a, b]);
      ...
      const first = list();
      src.set([a, { id: 2 }]); // b replaced with new object
      expect(list()).not.toBe(first);
    });
  });

  it('returns new array reference when an item is added', () => {
    createRoot(() => {
      const a = { id: 1 };
      const src = signal([a]);
      const list = stableList(() => src());
  ...
  it('returns new array reference when an item is removed', () => {
    createRoot(() => {
      const a = { id: 1 };
      const b = { id: 2 };
      const src = signal([a, b]);
  ...
  it('does not notify downstream effect when refs are stable', () => {
    createRoot(() => {
      const a = { id: 1 };
      const b = { id: 2 };
      const src = signal([a, b]);
      ...
      // Replace with new array, same refs — stableList returns same reference
      src.set([a, b]);
      expect(runs).toBe(1); // effect did NOT re-run
    });
  });

  it('notifies downstream effect when an item ref changes', () => {
    createRoot(() => {
      const a = { id: 1 };
      const src = signal([a]);
      const list = stableList(() => src());
      ...
      src.set([{ id: 1 }]); // new object, different reference
      expect(runs).toBe(2);
    });
  });

  it('works with a filter — stable when filtered result is unchanged', () => {
    createRoot(() => {
      const tasks = signal([
        { id: 1, projectId: 'p1' },
        { id: 2, projectId: 'p2' },
      ...
      const projectTasks = stableList(() => tasks().filter((t) => t.projectId === 'p1'));

      const first = projectTasks();
      expect(first).toHaveLength(2);

      // Add a task for a different project — items in 'p1' filter result are same refs
      const p2Task = { id: 4, projectId: 'p2' };
      const prev = tasks();
      tasks.set([...prev, p2Task]);
      expect(projectTasks()).toBe(first); // p1 items unchanged — same reference returned
    });
  });
});
