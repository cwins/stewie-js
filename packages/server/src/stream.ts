...
function serializeAttrs(props: Record<string, unknown>): string {
  ...
  for (const [key, rawValue] of Object.entries(props)) {
    ...
    if (value === true) { out += ` ${attrName}`; continue }
...
async function streamNode(node: unknown, opts: StreamOpts): Promise<void> {
  ...
  if (type === (Switch as unknown)) {
    ...
    for (const child of children as JSXElement[]) {
      ...
      const mp = child.props as { when: unknown; children: JSXElement | ((v: unknown) => JSXElement) }
      const when = typeof mp.when === 'function' ? (mp.when as () => unknown)() : mp.when
      if (when) {
        const content =
          typeof mp.children === 'function'
            ? (mp.children as (v: unknown) => JSXElement)(when)
            : mp.children
  ...
  return new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        const contextSnapshot: ContextSnapshot = new Map([
