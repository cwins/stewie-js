...
const ROLE_SELECTOR: Record<string, string> = {
  ...
  link: 'a',
  textbox:
    'input:not([type]), input[type="text"], input[type="email"], input[type="search"], input[type="url"], input[type="tel"], textarea',
...
function queryRole(container: Element, role: string, options?: { name?: string }): ElementHandle | null {
...
async function pollFor(query: () => ElementHandle | null, timeout = 1000): Promise<ElementHandle> {
...
export function mount(component: JSXElement | (() => JSXElement | null), options?: MountOptions): MountResult {
  ...
  return {
    ...
    findByText: (text: string, opts?: { timeout?: number }) => pollFor(() => queryText(container, text), opts?.timeout),
    findByTestId: (id: string, opts?: { timeout?: number }) => pollFor(() => queryTestId(container, id), opts?.timeout),
