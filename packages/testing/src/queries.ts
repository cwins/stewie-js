...
export function findByRole(html: string, role: string, options?: { name?: string }): ElementHandle | null {
  ...
  for (const tag of tags) {
    ...
    for (const el of elements) {
      ...
      if (options?.name) {
        ...
        const matchesName = (ariaLabel && ariaLabel.includes(options.name)) || hasText(el, options.name);
