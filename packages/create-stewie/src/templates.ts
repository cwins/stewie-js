...
export function generateFiles(ctx: TemplateContext): Array<{ path: string; content: string }> {
  ...
  const dependencies: Record<string, string> = {
    '@stewie-js/core': '^0.7.0',
    '@stewie-js/vite': '^0.7.0'
  };
  if (ctx.mode === 'ssr') {
    dependencies['@stewie-js/server'] = '^0.7.0';
    if (ctx.ssrRuntime === 'bun') {
      dependencies['@stewie-js/adapter-bun'] = '^0.7.0';
    } else {
      dependencies['@stewie-js/adapter-node'] = '^0.7.0';
    }
  }
  if (ctx.includeRouter) {
    dependencies['@stewie-js/router'] = '^0.7.0';
  }
  ...
  const devDependencies: Record<string, string> = {
    ...
    vite: '^7.0.0',
    vitest: '^4.0.0',
    '@stewie-js/testing': '^0.7.0',
    '@stewie-js/devtools': '^0.7.0'
  };
